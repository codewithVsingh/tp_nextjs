...
export default function AdminCities() {
  ...
  const cityData = useMemo(() => {
    // 0. Pre-map leads for fast lookup
    const leadCityMap: Record<string, string> = {};
    leads.forEach(l => { if (l.id) leadCityMap[l.id] = l.city || "Other"; });
    ...
    // Process Revenue
    payments.forEach(p => {
      // Robust city resolution: try p.city (new schema), then leadCityMap (old schema fallback)
      const rawCity = p.city || leadCityMap[p.lead_id];
      const city = (rawCity || "Other").trim().toLowerCase();
