...
import { resolveTemperature, getTemperatureColor, getTemperatureEmoji } from "@/modules/shared/logic/leadScoring";
...
export default function AdminAssignments() {
  ...
  const fetchLeads = useCallback(async () => {
    ...
    try {
      ...
      let q = supabase.from("leads").select("id, name, phone, class_level, city, subjects, status, lead_temperature, lead_score, budget", { count: "exact" })
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[300px] xl:w-[340px] shrink-0 flex flex-col border-r border-gray-200 bg-[#F4F6FA] overflow-hidden">
          ...
          {/* Lead pagination */}
          {leadTotalPages > 1 && (
            <div className="flex items-center justify-between px-3 py-3 border-t border-gray-200 bg-white shrink-0">
              <Button 
                variant="ghost" 
                size="sm" 
                disabled={leadPage === 1} 
              ...
              <Select value={leadPage.toString()} onValueChange={v => setLeadPage(parseInt(v))}>
                <SelectTrigger className="h-7 w-[75px] text-[10px] font-bold bg-gray-50 border-gray-200">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {Array.from({ length: leadTotalPages }, (_, i) => (
                    <SelectItem key={i+1} value={(i+1).toString()} className="text-[10px]">
                      {i+1}/{leadTotalPages}
                    </SelectItem>
                  ))}
                </SelectContent>
                ...
                {loadingA ? (
                  ...
                  <div className="space-y-2">
                    ...
                    {Math.ceil(assnTotal / assnPageSize) > 1 && (
                      <div className="flex items-center justify-between px-6 py-4 bg-white border border-slate-200 rounded-3xl shadow-sm mt-8">
                        <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">
                          {((assnPage - 1) * assnPageSize) + 1}-{Math.min(assnPage * assnPageSize, assnTotal)} <span className="mx-1">/</span> {assnTotal}
                        </p>
                        <div className="flex items-center gap-3">
                          ...
                          <Select 
                            value={assnPage.toString()} 
                            onValueChange={(val) => setAssnPage(parseInt(val))}
                          >
                            <SelectTrigger className="h-10 w-[110px] text-[12px] font-black bg-slate-50 border-slate-200 rounded-xl uppercase tracking-widest">
                              <SelectValue placeholder={`Page ${assnPage}`} />
                            </SelectTrigger>
                            <SelectContent className="rounded-xl border-slate-200">
                              {Array.from({ length: Math.ceil(assnTotal / assnPageSize) }, (_, i) => (
                          ...
                          <Button 
                            variant="ghost" 
                            size="sm" 
                            disabled={assnPage === Math.ceil(assnTotal / assnPageSize)} 
                            onClick={() => setAssnPage(p => p + 1)} 
                            className="h-10 px-4 text-[12px] font-black uppercase tracking-widest hover:bg-slate-50"
                          >
                            Next
