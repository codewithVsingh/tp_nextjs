  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [leadsRes, payRes, demoRes] = await Promise.all([
        supabase.from("leads").select("id, name, phone, class_level, city, subjects, status, created_at, source_page, source_cta, area"),
        supabase.from("payments").select("*").eq("status", "Paid"),
        supabase.from("demo_bookings").select("id, name, phone, class_level, city, subjects, status, created_at"),
      ]);
